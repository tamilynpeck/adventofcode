class BinaryDiagnostic:
    def __init__(self, input):
        ...
        self.life_support_rating = int(self.oxygen_generator_rating, 2) * int(
            self.c02_scrubber, 2
        )
    ...
    def calculate(self):
        ...
        for position in range(0, loops):
            ...
            oxygen_chars = [
                char for row in remaining_inputs_oxygen for char in row[position]
            ]
            ...
            if len(remaining_inputs_oxygen) > 1:
                remaining_inputs_oxygen = [
                    row
                    for row in remaining_inputs_oxygen
                    if row[position] == oxygen_most_common
                ]
            if len(remaining_inputs_c02) > 1:
                remaining_inputs_c02 = [
                    row
                    for row in remaining_inputs_c02
                    if row[position] == c0x_least_common
                ]
        ...
        self.oxygen_generator_rating = (
            remaining_inputs_oxygen[0] if remaining_inputs_oxygen else None
        )
